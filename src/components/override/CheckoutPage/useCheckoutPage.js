...
export const useCheckoutPage = props => {
    ...
    let is_virtual = true;
    cartItems.map(({__typename, giftcard_options}) => {
        if (__typename !== 'MpGiftCardCartItem' && __typename !== 'DownloadableCartItem' && __typename !== 'VirtualCartItem') {
            is_virtual = false;
        }
        if(giftcard_options) {
            for(let i = 0; i < giftcard_options.length; i++) {
                if(giftcard_options[i].code === "delivery_method" && giftcard_options[i].value === "4") {
                    is_virtual = false;
                    break;
                }
            }
